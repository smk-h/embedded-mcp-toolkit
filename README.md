...
#### <a id="section_exec_timeout">4.5 重要机制：exec 的提示符检测与超时熔断</a>
...
理解这个机制非常重要——它决定了一条命令是否会被自动发 Ctrl+C，以及什么时候会被发。**用错可能导致重启/烧写等关键命令被意外中断，破坏设备启动流程。**

【**机制流程**】

每条命令进入 exec 后，按以下顺序处理：

（1）**前置冲刷**：发命令前先清空缓冲区残留，避免上次未终止命令的输出混入

（2）**发送命令并轮询**：在 `maxDuration`（默认 10000ms）内持续读取输出

（3）**结束判定（二选一）**：
- **正常结束**：检测到 shell 提示符（Android `:/ $` / `:/ #`、Linux `$` / `#` / `>`、U-Boot `=>`，支持设备配置 `promptPattern` 覆盖）→ 立即返回，**不发 Ctrl+C**
- **超时熔断**：到 `maxDuration` 仍未检测到提示符 → **自动发一次 Ctrl+C**（`\x03`）尝试终止命令，返回已收集输出并标注 `[timed-out: collected Xms of output, Ctrl+C sent]`

【**什么时候会发 Ctrl+C**】

| 命令类型 | 示例 | 是否检测到提示符 | 是否发 Ctrl+C |
|----------|------|------------------|---------------|
| 瞬时命令 | `ls`、`echo hi`、`getprop`、`uname -a` | ✅ 是 | ❌ 不发 |
| 可完成的长命令 | `sleep 5; echo done`（≤ maxDuration） | ✅ 是（done 后出提示符） | ❌ 不发 |
| 前台常驻命令 | `logcat`、`top`、`ping` | ❌ 否（命令不退出） | ✅ 发（这是预期行为，取 N 秒采样） |
| **对 Ctrl+C 敏感的长启动命令** | **`reboot`、`reboot bootloader`、烧写/刷机、固件升级** | ❌ 否（重启过程中无 shell 提示符） | ⚠️ **会发，且可能破坏启动流程** |

> **关键点**：熔断发 Ctrl+C 是**无条件的**——只要到时间没看到 shell 提示符就发，**不区分「常驻采样」和「设备正在重启」**。对 `logcat` 是好事（自动停），对 `reboot` 是灾难（可能中断 autoboot、引导菜单、烧写过程）。

【**⚠️ 特别警告：reboot 等命令禁止用 exec 跑默认超时**】

`reboot`、`reboot bootloader`、固件烧写、`kexec`、UBI/ mmc 烧录等命令有两个特点：① 执行时间长（几十秒到数分钟），② 过程中对 Ctrl+C 极度敏感（autoboot 倒计时、bootloader 菜单、烧写中断、启动脚本里「Press Ctrl+C to stop …」之类的等待点都可能让设备进入异常状态）。

这类命令一旦在 exec 中超时，熔断发的 Ctrl+C 恰好可能落在上述敏感点上，导致启动流程被中止、设备卡在中途。**正确用法（write+read 组合 / exec 显式大超时 / 如何向 AI 下达指令）见 [常见问题 3. 重启被中断？](#section_reboot_interrupted)。**
...
超时熔断返回的 `[timed-out: collected Xms of output, Ctrl+C sent]` 是**中性采样结果，不是异常**——它告诉调用方「到时间了，输出已收集完毕」。对 `logcat` 取 N 秒日志、`top` 采样这类场景，这就是预期行为，AI 不应将其视为命令出错。**真正需要警惕的是上文对 Ctrl+C 敏感的长启动命令。**
...
> 完整机制说明见 [4.5 重要机制：exec 的提示符检测与超时熔断](#section_exec_timeout)。
