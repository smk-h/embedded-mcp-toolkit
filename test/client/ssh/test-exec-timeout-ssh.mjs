/**
 * =====================================================
 * exec 超时与常驻命令识别 端到端测试 —— SSH 通道
 *
 *   验证 ch13 的「常驻命令分类 + 双超时分支」机制（ssh_shell_exec）：
 *     1. 普通瞬时命令正常返回，无超时标注
 ...
 *   运行：node test/client/ssh/test-exec-timeout-ssh.mjs
 * ======================================================
 */
...
async function main() {
  ...
  console.log("╔══════════════════════════════════════════════╗");
  console.log(`║  exec 超时测试 —— SSH 通道 (${device.padEnd(12)}) ║`);
  console.log("╚══════════════════════════════════════════════╝");

  // 1. 连接 MCP 服务器
  let client;
  try {
    const conn = await connect({ name: "test-exec-ssh", env: serverEnv });
    client = conn.client;
    pass("MCP 服务器连接成功");
  } catch (err) {
  ...
  try {
    ...
    await probeCommands(client, SSH_TOOLS, sessionId);

    // 4. 运行全部 exec 测试用例
    await runExecTestCases(client, SSH_TOOLS, sessionId);
  } finally {
    // 4. 关闭会话
    if (sessionId) {
      try {
        await callClose(client, SSH_TOOLS, sessionId);
        pass("SSH 会话已关闭");
      } catch (err) {
