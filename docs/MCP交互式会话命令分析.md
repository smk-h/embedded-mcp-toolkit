...
命令执行的核心编排逻辑集中在 `src/sdk/exec/exec-runner.ts`（统一编排器）、`src/sdk/exec/prompt-detector.ts`（提示符检测）、`src/sdk/exec/resident-detector.ts`（常驻命令分类）与 `src/sdk/exec/send-ctrl.ts`（控制字符发送）。三个通道的传输层类继承自 `src/sdk/transports/base-shell.ts` 的统一基类，通过依赖注入将通道差异交给编排层处理，实现「机制统一、通道无关」的设计。
...
- **传输层**（`src/sdk/transports/`）：负责建立物理连接、收发原始字节、维护输出缓冲区，提供统一的 `open / write / read / drain / close` 五方法契约
