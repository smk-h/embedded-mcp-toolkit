...
export function printTunnelSummary(state: TunnelState): void {
  ...
  log.message(
    `    ssh -i ~/.ssh/id_mcp_server -o ProxyCommand="cloudflared access ssh --hostname ${state.domain}" ${winUser}@${state.domain}`
  );
  log.message(
    "    提示: 新域名 DNS 传播约需 1 分钟,立即连接可能报 no such host;免密需先在远端生成密钥并把公钥写入本机 authorized_keys"
  );
}
