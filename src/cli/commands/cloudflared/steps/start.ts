 ...
 *
 * 流程：探测 cloudflared → 防重复启动预检 → spawn detached 后台进程 →
 ...
 * @details 幂等设计：已运行时直接展示现有域名，不重复拉起进程；
 *          残留状态文件（进程已退出）先清理再重启。域名提取依赖轮询，
 *          超时但进程存活时不算失败——status 会从日志补录域名。
...
export async function doStart(url: string = DEFAULT_TUNNEL_URL): Promise<boolean> {
  ...
  mkdirSync(resolve(process.cwd(), TUNNEL_DIR_REL), { recursive: true });
  const logFile = resolve(process.cwd(), LOG_FILE_REL);
  let pid: number;
  try {
