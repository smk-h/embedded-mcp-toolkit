...
| `9` | 清理 authorized_keys 失效公钥 | 列出全部公钥条目，勾选批量删除 |
| `0` | 退出 | 结束命令 |
...
#### 2.5 清理 authorized_keys 失效公钥（菜单 `[9]`）

CNB 等临时容器环境每次重建都会生成新密钥对，新公钥经菜单 `[4]` 不断追加进 `~/.ssh/authorized_keys`，旧容器的公钥随之失效堆积。菜单 `[9]` 用于批量清理这些残留：

1. 列出 `~/.ssh/authorized_keys` 中全部公钥条目，每条展示**密钥类型 + SHA256 指纹（与 `ssh-keygen -lf` 一致）+ 行尾注释（通常是 `user@host`）**；
2. 与当前 `.embedded/ssh/id_mcp_server.pub` 内容一致的条目会标记 `(当前生效)`；
3. 空格勾选待删条目、Enter 确认，二次确认后批量删除；勾选了当前生效条目时会额外警告（删除后远端将无法免密登录）；
4. 未勾选的条目（含手工添加的其它公钥）保持原样不受影响。

> 行尾注释可能重复（如多个容器同名同主机名），此时以 SHA256 指纹区分条目。
